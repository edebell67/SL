...
with tab2:
    ...
    if not df_weekly.empty:
        ...
        with st.sidebar:
            ...
            selected_tradeable = st.selectbox('Select Tradeable', ['All'] + df_weekly['tradeable'].unique().tolist(), key='tradeable_2')
            selected_signal = st.selectbox('Select Signal', ['All'] + df_weekly['signal'].unique().tolist(), key='signal_2')
        ...
        if show_net_graph:
            if 'update_time' in df_weekly.columns and 'net' in df_weekly.columns:
                df_weekly['update_time'] = pd.to_datetime(df_weekly['update_time'])
                fig = px.line(df_weekly, x='update_time', y='net', title='Net Over Time', markers=True)
