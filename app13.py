...
def display_scrolling_trades(data):
    if data.empty:
        st.write("No data available")
    else:
        trade_texts = [f"<span>{row['entry_datetime']} - {row['trade_desc']}</span><br>" for _, row in data.iterrows()]
...
tab1, tab2, tab3, tab4 = st.tabs(["Current Trades Analysis", "Weekly Tradeable Summary", "Live Trade Descriptions", "Detailed Trade Streams"])
...
with tab4:
    # Fetch data from your designated source
    df_tab4 = get_data("your_api_endpoint_for_tab4_data")  # Replace with actual API endpoint if different

    # Check if data is not empty and proceed
    if not df_tab4.empty:
        # Filter data based on sidebar ID selection applicable across all sub-columns
        selected_id_tab4 = st.sidebar.selectbox('Select ID for Detailed Trade Streams', ['All'] + sorted(df_tab4['id'].unique().tolist()), key='id_tab4')

        if selected_id_tab4 != 'All':
            df_tab4 = df_tab4[df_tab4['id'] == selected_id_tab4]

        # Set up columns for different trade and tradeable conditions
        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.subheader("Buys with Tradeable -1")
            buy_minus_one = df_tab4[(df_tab4['trade'] == 'buy') & (df_tab4['tradeable'] == -1)]
            display_scrolling_trades(buy_minus_one)

        with col2:
            st.subheader("Buys with Tradeable 0")
            buy_zero = df_tab4[(df_tab4['trade'] == 'buy') & (df_tab4['tradeable'] == 0)]
            display_scrolling_trades(buy_zero)

        with col3:
            st.subheader("Sells with Tradeable -1")
            sell_minus_one = df_tab4[(df_tab4['trade'] == 'sell') & (df_tab4['tradeable'] == -1)]
            display_scrolling_trades(sell_minus_one)

        with col4:
            st.subheader("Sells with Tradeable 0")
            sell_zero = df_tab4[(df_tab4['trade'] == 'sell') & (df_tab4['tradeable'] == 0)]
            display_scrolling_trades(sell_zero)
